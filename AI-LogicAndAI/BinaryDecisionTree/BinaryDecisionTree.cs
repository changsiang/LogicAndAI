...
namespace AI_LogicAndAI.BinaryDecisionTree
{
    public class BinaryDecisionTree
    {
        ...
        public static BinaryDecisionTree TreeFromFormula(Formula f)
        {
        ...
        private static BinaryDecisionTree TreeBuilder(Formula f, IEnumerable<Variable> variables, int varIndex, string path)
        {
            ...
            return new BinaryDecisionTree(varIndex, TreeBuilder(f, variables, varIndex + 1, path + "0"), TreeBuilder(f, variables, varIndex + 1, path + "1"));
        }
    }
}
